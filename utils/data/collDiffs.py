...
class collDiffs:
    ...
    def collDf(coll_sets, labels=None, slice="slice", collocate="colloc", node="node"):
        ...
        coll_df = pd.DataFrame.from_records(
            coll_data, columns=[collocate, slice, "rank"]
        )
        coll_df[[collocate, slice]] = coll_df[[collocate, slice]].astype(
            "category"
        )
        return coll_df

    # get n terms from the set each word shares the most / least collocates
    def getNTop(collDf, top=10, ascending = False, slice="slice", collocate="colloc", node="node"):
            # if top = -1 retrieves all
            inters_counts = collDiffs.all2all(
                [group[collocate] for name, group in collDf.groupby(slice)]
            )[2]
    ...
    def plotCollDf(collDf, type="heatmap", show=True, slice="slice", collocate="colloc"):
